...
<div class="shrink-0 sticky top-0 z-10 bg-[#0c1117] border-b border-white/10">
  <div class="px-3 pt-3 pb-2 flex items-center gap-2">
    ...
    <div class="font-semibold tracking-tight">AI Chat</div>
    <button id="themeToggle" class="ml-auto text-xs px-2 py-1 rounded border border-white/10 hover:bg-white/5">Theme</button>
  </div>
  ...
  <div class="px-3 pb-3 space-y-2">
    @if(($projects->count() ?? 0) > 0)
      ...
      <form method="POST" action="{{ route('sessions.store', $firstProject) }}" class="w-full">
        ...
        <button id="newChatBtn"
          class="w-full flex items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold transition
    ...
    <div class="mt-2 relative">
      <input id="chatSearch" placeholder="Search chats"
             class="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-sm placeholder:text-gray-500
...
<div id="sessionList" class="flex-1 overflow-y-auto min-h-0 px-2 py-2 space-y-1">
  @forelse($sessionsList as $s)
    ...
    <div class="group rounded-lg border border-white/5 {{ $active ? 'bg-white/10' : 'bg-white/[.03] hover:bg-white/[.06]' }} transition">
      <div class="flex items-center gap-2 pl-2 pr-1">
        ...
        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
          ...
          <button type="button"
                  ...
                  data-rename
                  data-url="{{ route('sessions.update', $sid) }}"
                  data-title="{{ $title }}"
                  title="Rename">
          ...
          {{-- Delete session --}}
          <form method="POST" action="{{ route('sessions.destroy', $sid) }}"
            ...
            <button class="p-1 rounded hover:bg-white/10" title="Delete">
              <svg class="w-4 h-4 text-red-400" viewBox="0 0 24 24" fill="none">
                <path stroke="currentColor" stroke-width="2" d="M19 7H5m3 0V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m1 0v12a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V7"/>
              </svg>
            </button>
          </form>
        </div>
...
<div id="sidebarFooter" class="mt-auto bg-[#0c1117] border-t border-white/10">
  <div class="px-3 py-2 space-y-2">
    <form method="POST" action="{{ route('logout') }}">
      @csrf
      <button class="w-full inline-flex items-center gap-2 text-sm rounded-lg px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10">
        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none">
          <path stroke="currentColor" stroke-width="2" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6A2.25 2.25 0 0 0 5.25 5.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12"/>
        </svg>
        Logout
      </button>
    </form>
