...
<body class="h-full text-gray-100 bg-[#0b0f15]" style="background-color: #0b0f15;">
 ...
 <div class="flex flex-col min-h-0 min-w-0 overflow-hidden">
    ...
    <aside id="artifactsSidebar" class="hidden lg:flex flex-col bg-[#0d1117]/80 backdrop-blur-xl border-l border-white/10 translate-x-full transition-transform duration-300 fixed inset-y-0 right-0 w-full z-30 lg:relative lg:translate-x-0 group">
      <div class="flex flex-col h-full">
        ...
        <div class="h-14 border-b border-white/10 flex items-center justify-between px-4 shrink-0 bg-white/5">
          <div class="flex items-center gap-2">
            ...
            <div class="flex flex-col">
              <span id="artifactTitle" class="text-sm font-semibold text-gray-100 truncate max-w-[200px]">Artifact Preview</span>
              <span id="artifactSubtitle" class="text-[10px] text-emerald-500/70 font-bold uppercase tracking-widest">Premium Studio</span>
            </div>
          </div>
          <div class="flex items-center gap-1">
             <button id="artifactCopy" class="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-all" title="Copy Code">
             ...
             <button id="artifactDownload" class="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-all" title="Download">
             ...
             <button id="artifactClose" class="p-2 hover:bg-red-500/20 rounded-lg text-gray-400 hover:text-red-400 transition-all" title="Close">
        ...
        <div id="artifactContent" class="flex-1 overflow-auto bg-[#0b0f15] relative p-6">
           <div class="h-full w-full flex flex-col gap-4">
             {{-- Iframe for HTML/JS --}}
             <div id="iframeWrapper" class="flex-1 rounded-xl bg-white shadow-2xl overflow-hidden hidden border border-white/5 ring-1 ring-white/10">
               <iframe id="artifactIframe" class="w-full h-full border-none" sandbox="allow-scripts allow-same-origin"></iframe>
             </div>
             {{-- Div for Mermaid --}}
             <div id="artifactMermaid" class="flex-1 rounded-xl bg-white/5 hidden overflow-auto flex items-center justify-center p-8 border border-white/5"></div>
           </div>
           
           {{-- Empty State --}}
           <div id="artifactEmpty" class="absolute inset-0 flex flex-col items-center justify-center text-gray-500 gap-4">
    ...
    {{-- Artifact Toggle --}}
    <button id="artifactToggle" class="fixed right-0 top-1/2 -translate-y-1/2 z-40 px-3 py-6 bg-[#0d1117] border border-white/10 border-r-0 rounded-l-xl text-emerald-500 hover:bg-emerald-500/10 transition-all shadow-2xl hidden group" title="Open Preview">
       <div class="flex flex-col items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6"/></svg>
          <span class="[writing-mode:vertical-lr] text-[10px] font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">PREVIEW</span>
       </div>
    </button>
  </div>
