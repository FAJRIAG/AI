    ...
    <aside class="w-72 border-r border-gray-200 bg-white p-3 space-y-3">
      <div class="flex items-center justify-between">
        ...
        <form method="POST" action="{{ route('projects.store') }}">
          ...
          <button class="inline-flex items-center gap-1 px-2 py-1 text-sm rounded bg-indigo-600 text-white hover:bg-indigo-700">+ Project</button>
        </form>
      </div>
          ...
          <div class="rounded-md border border-gray-200">
            <div class="px-2 py-1.5 flex items-center justify-between">
              <form method="POST" action="{{ route('projects.update',$p) }}" class="flex-1">
                @csrf @method('PATCH')
                <input name="name" value="{{ $p->name }}" class="w-full bg-transparent text-sm font-medium text-gray-800 focus:outline-none"/>
              </form>
              <form method="POST" action="{{ route('projects.destroy',$p) }}" onsubmit="return confirm('Delete project?')" class="pl-2">
                @csrf @method('DELETE')
    ...
    <main class="flex-1 flex flex-col">
      <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white">
        <div class="flex items-center gap-2">
          <button x-on:click="toggleTheme" class="px-2 py-1 text-xs rounded border border-gray-300 text-gray-700">Light/Dark</button>
          @if($currentSession)
          <form method="POST" action="{{ route('sessions.update',$currentSession) }}" class="flex items-center">
            @csrf @method('PATCH')
            <input name="title" value="{{ $currentSession->title }}" class="px-2 py-1 rounded text-sm bg-transparent border border-transparent focus:border-indigo-400 text-gray-800"/>
          </form>
          @endif
        </div>
        @if($currentSession)
        <form method="POST" action="{{ route('sessions.destroy',$currentSession) }}" onsubmit="return confirm('Delete chat?')">
          @csrf @method('DELETE')
        ...
        @if($currentSession)
          ...
          <div id="typing" class="max-w-3xl mx-auto px-4 pb-4 hidden">
            <div class="flex gap-2 items-start">
              <img src="https://ui-avatars.com/api/?name=AI" class="w-7 h-7 rounded-full" alt="AI">
              <div class="bg-gray-100 rounded-lg px-3 py-2 text-sm text-gray-700">AI is typing…</div>
            </div>
          </div>
        @else
      ...
      <div class="border-t border-gray-200 bg-white">
        <div class="max-w-3xl mx-auto p-3 flex gap-2">
          <textarea x-model="input" x-on:keydown.enter.prevent="submit" placeholder="Tulis pesan…" class="flex-1 resize-none rounded border border-gray-300 bg-white text-gray-800 p-2"></textarea>
          <button x-bind:disabled="loading || !input.trim()" x-on:click="submit" class="px-4 py-2 rounded bg-indigo-600 text-white disabled:opacity-60">Kirim</button>
        </div>
      </div>
      @endif
    </main>
...
function chatApp(){
  return {
    ...
    toggleTheme(){ const c=document.documentElement.classList; c.toggle('dark'); localStorage.setItem('theme', c.contains('dark')?'dark':'light'); },
    ...
    renderMsg(m){
      ...
      avatar.src = m.role==='user' ? 'https://ui-avatars.com/api/?name=U' : 'https://ui-avatars.com/api/?name=AI';
    ...
    async submit(){
      ...
      if(!resp.ok || !resp.body){ this.addMsg({role:'assistant',content:'(Error AI)'}); this.loading=false; this.showTyping(false); return; }
