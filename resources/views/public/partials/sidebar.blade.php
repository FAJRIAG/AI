...
<aside class="sidebar hidden md:flex flex-col h-full min-h-0 bg-[#0c1117] text-gray-200 border-r border-white/10">
  ...
  <div class="shrink-0 sticky top-0 z-10 bg-[#0c1117] border-b border-white/10">
    <div class="px-3 pt-3 pb-2 flex items-center gap-2">
      <div class="size-8 rounded bg-emerald-600 grid place-items-center font-bold">AI</div>
      <div class="font-semibold tracking-tight">AI Chat</div>
      <button id="themeToggle" class="ml-auto text-xs px-2 py-1 rounded border border-white/10 hover:bg-white/5">
        Theme
      </button>
    </div>
      ...
      <form method="POST" action="{{ route('public.new') }}" class="w-full">
        ...
        <button id="newChatBtn"
          class="w-full flex items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold transition
                 bg-black text-white border-black hover:bg-[#111]"
          type="submit">
